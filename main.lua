...
-- // CRÉATION DE L'INTERFACE (GUI) DU LOADER //
if PlayerGui:FindFirstChild("D3X_SECURITY") then PlayerGui.D3X_SECURITY:Destroy() end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "D3X_SECURITY"
ScreenGui.IgnoreGuiInset = true
ScreenGui.Parent = PlayerGui

-- Effet de Flou dans le jeu
local Blur = Instance.new("BlurEffect")
Blur.Size = 0
Blur.Parent = Lighting

-- Cadre Principal (Carte)
local MainFrame = Instance.new("Frame")
...
AvatarImg.Position = UDim2.new(0.5, -35, 0, 25)
AvatarImg.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
AvatarImg.BackgroundTransparency = 1
AvatarImg.Parent = MainFrame

local AvatarCorner = Instance.new("UICorner")
AvatarCorner.CornerRadius = UDim.new(1, 0) -- Cercle parfait
...
BarFill.BackgroundColor3 = Color3.fromRGB(170, 0, 255) -- Couleur Violet Néon
BarFill.BorderSizePixel = 0
BarFill.Parent = BarBg
Instance.new("UICorner", BarFill).CornerRadius = UDim.new(1,0)

-- Effet de brillance sur la barre (Gradient)
...
task.spawn(function()
    -- 1. Intro : Flou + Zoom In
    TweenService:Create(Blur, TweenInfo.new(1), {Size = 20}):Play()
    MainFrame.Size = UDim2.new(0, 0, 0, 0)
    TweenService:Create(MainFrame, TweenInfo.new(0.6, Enum.EasingStyle.Back), {Size = UDim2.new(0, 350, 0, 200)}):Play()

    wait(0.5)

    -- 2. Chargement de l'avatar
    StatusLbl.Text = "Fetching User Data..."
    -- CORRECTION ICI : Inversion des variables de pcall
    local isReady, content = pcall(function()
        return Players:GetUserThumbnailAsync(Player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)
    end)

    if isReady then 
        AvatarImg.Image = content 
        -- Petite anim sur l'avatar
        AvatarImg.Size = UDim2.new(0,0,0,0)
        TweenService:Create(AvatarImg, TweenInfo.new(0.4, Enum.EasingStyle.Bounce), {Size = UDim2.new(0, 70, 0, 70)}):Play()
    end
    ...
    StatusLbl.Text = "Verifying Whitelist..."
    local isAllowed = false

    -- Vérification ID
    for _, id in pairs(WHITELIST_IDS) do
    ...
    if isAllowed then
        ...
        UIStroke.Color = Color3.fromRGB(0, 255, 100) -- Bordure verte
        TweenService:Create(BarFill, TweenInfo.new(0.5), {Size = UDim2.new(1, 0, 1, 0)}):Play()

        local sound = Instance.new("Sound")
        sound.SoundId = "rbxassetid://6895079853" -- Son de succès futuriste
        ...
        UIStroke.Color = Color3.fromRGB(255, 50, 50) -- Bordure rouge
        BarFill.BackgroundColor3 = Color3.fromRGB(255, 50, 50)

        -- Effet de secousse (Shake)
        for i = 1, 5 do
            MainFrame.Position = UDim2.new(0.5, math.random(-5, 5), 0.5, math.random(-5, 5))
            wait(0.05)
        end
        MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
